...
module _ {Σ : Sig} {P : Prg Σ Σ} {M : Meths Σ Σ} {rt : Type} {rv : Val rt} where
  ...
  mutual
    ...
    corrCall evF evBody ret ev = {!!}
    corrCall evF evBody cont ev = {!!}
    ...
    corrStms : ∀ {Γ Δ Δ'} {ss : Stms Σ rt Γ Δ Δ'} {γ γ'} {r : Res rt} {Φ} {φ : Frame Φ}
      ...
      → CREval M ƛ rv k  (γ' , φ)
      → CREval M ƛ rv cr (γ , φ)
    corrStms evNil ev = ev
    corrStms (evCont evS evSS) ev = corrStm evS (corrStms evSS ev)
    corrStms (evRet evR) ev = {!x!}
    ...
    corrStm : ∀ {Γ t} {s : Stm Σ rt Γ t} {γ γ'} {r : Res rt} {Φ} {φ : Frame Φ}
      ...
      → CREval M ƛ rv k  (γ' , φ)
      → CREval M ƛ rv cr (γ , φ)
    corrStm (evReturn evE) = corrComment ∘ corrExp evE ∘ corrReturn
    corrStm (evExp evE)    = corrComment ∘ corrExp evE ∘ corrPop
    corrStm evDecl         = corrComment ∘ corrDecl
    corrStm (evInit evE)   = corrComment ∘ corrExp evE ∘ corrInit
    corrStm (evBlock evSS) ev = {!!}
    corrStm (evWhileDone evF) ev = corrComment {!!}
    corrStm (evWhileStep evT evS evS') ev = corrComment {!!}
    corrStm (evWhileRet evT evS) ev = corrComment {!!}
    ...
    corrReturn (ev□Block (evBB evJF evCtrl)) = ev□Block (evBB [] (evReturn {!!}))
    corrReturn (ev□Goto ev)                  = ev□Block (evBB [] (evReturn {!!}))
