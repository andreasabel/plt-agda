...
module _ {Σ : Sig} {rt : Type} where
  ...
  flat (fcGoto l) = ⊆-refl ∙ (λ τ → AllExt-id) ∙ λ τ → mkBB [] (bbGoto (⊆-lookup τ l))
  flat fcReturn   = ⊆-refl ∙ (λ τ → AllExt-id) ∙ λ τ → mkBB [] bbReturn
  ...
  flat (fcFix fc)
    with η ∙ bbs ∙ bb ← flat fc
      ...
      ∙ (λ τ → mkBB [] (bbGoto (⊆-lookup (⊆-trans η τ) here!)))
  ...
  flat (fcIfElse c fc₁ fc₂)
    ...
    η     = ⊆-trans η₁ ξ₁
    bs    : □ (λ Λ′ → BB Λ′ _) _
    bs  τ = mkBB [] (bbIfElse c (bb₁ (⊆-trans ξ₁ τ)) (bb₂ (⊆-trans ξ₂ τ)))
